...
void startearlysvcrunner(char * const *envp)
{
	while ((earlysvcpid =fork()) == -1)
	{
		strerr_warn2(FATAL, "unable to fork for early service runner pausing: ",
		             &strerr_sys);
		sleep(5);
	}
	if (!earlysvcpid)
	{
		/* child */
		prog[0] ="/sbin/runsvdir";
		prog[1] ="/run/svc/sysinit";
		prog[2] =0;
		...
		strerr_warn2(INFO, "start early service runner: ", 0);
		execve(*prog, (char *const *)prog, envp);
		strerr_die2sys(0, FATAL, "unable to start early service runner: ");
	}
}
...
int main (int argc, const char * const *argv, char * const *envp)
{
	...
	for (curstage =0; curstage < 4; curstage++)
	{
		...
		if (curstage > 0 && curstage != 3 && !earlysvcpid)
		{
			startearlysvcrunner(envp);
		}
		...
		for (;;)
		{
			...
			while ((child =wait_nohang(&wstat)) > 0)
				if (child == pid || child == earlysvcpid) break;
			if (child == -1)
			{
			...
			if (child == earlysvcpid && curstage > 0 && curstage != 3)
			{
				if (wait_exitcode(wstat) != 0)
				{
					if (wait_crashed(wstat))
						strerr_warn2(WARNING, "early service runner crashed: ", 0);
					else
						strerr_warn2(WARNING, "early service runner failed: ", 0);
				}
				earlysvcpid=0;
				startearlysvcrunner(envp);
				continue;
